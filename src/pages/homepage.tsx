...
const BLOOMS_ALL_CODES: Record<string, string> = {};
Object.values(BLOOMS_TAXONOMY).flat().forEach(({ code, text }) => {
    BLOOMS_ALL_CODES[code] = text;
});

function getBloomsClass(code: string): string {
    if (!code) return 'blooms-default';
    const prefix = code[0].toUpperCase();
    if (prefix === 'C') {
        const num = parseInt(code[1]);
        if (num >= 1 && num <= 6) return `blooms-c${num}`;
    }
    if (prefix === 'A') return 'blooms-a';
    if (prefix === 'P') return 'blooms-p';
    return 'blooms-default';
}

function getKnowledgeLabel(codes: string[]): string {
    if (!codes || codes.length === 0) return '—';
    const first = KNOWLEDGE_PROFILE.find(k => k.code === codes[0]);
    if (!first) return codes.join(', ');
    const suffix = codes.length > 1 ? ` +${codes.length - 1}` : '';
    return `${first.code}: ${first.text}${suffix}`;
}
...
const HomePage = () => {
    ...
    const [showReviewCard, setShowReviewCard] = useState<boolean>(true);
    ...
    // For PO coverage: which POs have at least one CO mapping, and how many
    ...
    const showModal = (title: string, message: string, onConfirm: () => void, confirmText = 'Confirm', confirmClass = 'btn-primary', showCancel = true) => {
        setModalTitle(title);
    ...
    const handleRemoveObjective = (blockId: string) => {
        ...
        showModal(
            ...
            () => {
                ...
                if (editingCOId === blockId) setEditingCOId(null);
    ...
    const handleDiscard = () => {
        showModal(
            'Discard Changes',
            'This will reload the saved objectives and discard any unsaved changes. Continue?',
            () => {
                setSelectedSession(prev => {
                    const s = prev;
                    setSelectedSession('');
                    setTimeout(() => setSelectedSession(s), 50);
                    return prev;
                });
            },
            'Discard',
    ...
    // Top-right of layout: Export Report button
    const topbarRight = (
    ...
    return (
        <Layout teacherName={teacherName} onLogout={handleLogout} page="homepage" title="Outcome Mapper" topbarRight={topbarRight}>
            <Head>
            ...
            <div className="section-card">
                ...
                <div className="section-card-body">
                    <div className="course-selection-grid">
                        ...
                        <div>
                            <label className="cs-field-label">Department</label>
                            <div className="custom-select">
                                <button type="button" className="custom-select-toggle disabled" disabled>
                        ...
                        {/* Academic Session */}
                        <div>
                            <label className="cs-field-label">Academic Session</label>
                        ...
                        {/* Select Course */}
                        <div>
                            <label className="cs-field-label">Select Course</label>
            ...
            <div className="section-card">
                <div className="section-card-header">
                    <div>
                        ...
                        <p className="section-card-subtitle">Map specific course objectives to academic standards.</p>
                    </div>
                    {selectedCourse && selectedSession && (
                        <button className="btn-add-co" onClick={createObjectiveBlock}>
                ...
                {isLoadingObjectives ? (
                    ...
                    <div className="no-course-state">
                        <div className="no-course-state-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="48" height="48" style={{ opacity: 0.3, color: 'var(--primary-color)' }}>
                                <path d="M4 6H2v14c0 1.1.9 2 2 2h14v-2H4V6zm16-4H8c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-1 9H9V9h10v2zm-4 4H9v-2h6v2zm4-8H9V5h10v2z" />
                            </svg>
                        </div>
                        <p>
                            {courses.length > 0
                                ? 'Select a course and session above to begin defining objectives.'
                    ...
                    <>
                        <div style={{ overflowX: 'auto' }}>
                            <table className="co-table">
                                <thead>
                                    <tr>
                                        <th style={{ width: 60 }}>CO ID</th>
                                        <th>Description</th>
                                        <th style={{ width: 160 }}>Bloom&apos;s Level</th>
                                        <th style={{ width: 180 }}>Knowledge Profile</th>
                                        <th style={{ width: 140 }}>PO Mapping (1–12)</th>
                                        <th style={{ width: 70 }}>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {courseObjectives.length === 0 ? (
                                        <tr className="co-empty-row">
                                            <td colSpan={6}>No course objectives yet. Click &ldquo;Add CO&rdquo; to get started.</td>
                                        </tr>
                                    ) : courseObjectives.map(obj => (
                                        editingCOId === obj.id ? (
                                            // ── Edit Row ──
                                            <tr key={obj.id} className="co-edit-row">
                                                <td colSpan={6}>
                                                    <div className="co-edit-panel">
                                                        <div className="co-edit-header">
                                                            <span className="co-edit-co-label">CO{obj.displayNumber} — Editing</span>
                                                            <button
                                                                type="button"
                                                        ...
                                                        {/* Description + PO */}
                                                        <div className="co-edit-main-row">
                                                        ...
                                                        {/* Taxonomy selectors */}
                                                        <div className="co-edit-taxonomy-grid">
                                                            {/* Bloom's */}
                                                        ...
                                                        {/* Edit row footer */}
                                                        <div className="co-edit-footer">
                                                            <div>
                                                                {courseObjectives.length > 1 && (
                                                                    <button
                                                                        type="button"
                                                                        className="co-edit-remove-btn"
                                            ...
                                            // ── View Row ──
                                            <tr key={obj.id} className="co-view-row">
                                                <td>
                                                    <div className="co-id-badge">CO{obj.displayNumber}</div>
                                                </td>
                                                <td>
                                                    {obj.description ? (
                                                        <div className="co-desc-text">{obj.description}</div>
                                                    ) : (
                                                        <span className="co-desc-empty">No description set</span>
                                                    )}
                                                </td>
                                                <td>
                                                    {obj.bloomsTaxonomy.length > 0 ? (
                                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.25rem' }}>
                                                            {obj.bloomsTaxonomy.map(code => (
                                                                <span key={code} className={`blooms-badge ${getBloomsClass(code)}`}>
                                                                    {code} ({BLOOMS_ALL_CODES[code] || code})
                                                                </span>
                                                            ))}
                                                        </div>
                                                    ) : (
                                                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.775rem' }}>—</span>
                                                    )}
                                                </td>
                                                <td>
                                                    <span className="knowledge-text">
                                                        {getKnowledgeLabel(obj.knowledgeProfile)}
                                                    </span>
                                                </td>
                                                <td>
                                                    {obj.programOutcome ? (
                        ...
                        <div className="co-metrics-row">
                            ...
                            <div className="co-metric-item">
                                ...
                                <div>
                                    <div className="co-metric-value">
                                        <span className={`validation-badge ${validationStatus.cls}`}>
                            ...
                            <div className="co-metric-item">
                                ...
                                <div>
                                    <div className="co-metric-value">Now</div>
                    ...
                    <div className="section-card-header">
                        <div>
                            ...
                            <p className="section-card-subtitle">
                                Distribution of Course Objectives across the 12 Program Outcomes.
                            </p>
                        </div>
                    </div>
                    <div className="section-card-body">
                        <div className="po-coverage-grid">
                            {poCoverage.filter(po => po.pct > 0 || po.count > 0).slice(0, 8).map(po => (
                                <div key={po.key} className="po-coverage-item">
                                    ...
                                    <div className="po-coverage-bar-wrap">
                                        <div
                                            className="po-coverage-bar-fill"
                                            style={{ width: `${Math.max(po.pct, po.pct > 0 ? 4 : 0)}%` }}
                                        />
                                    </div>
                                    <span className="po-coverage-pct">{po.pct}%</span>
                                </div>
                            ))}
                            {poCoverage.every(po => po.pct === 0) && (
                                <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', margin: 0 }}>
                                    No Program Outcomes mapped yet. Edit your COs to assign PO targets.
                                </p>
                            )}
                        </div>
                    </div>
            ...
            {selectedCourse && selectedSession && (
                <div className="page-action-bar">
                    ...
                    >
                        {isSaving ? (
                            <>
                                <svg className="animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" width="15" height="15">
            ...
            {selectedCourse && selectedSession && hasReviewRequired && showReviewCard && (
                <div className="review-card">
                    <div className="review-card-header">
                        <h3 className="review-card-title">Review Required</h3>
                        <div className="review-card-badge" style={{ cursor: 'pointer' }} onClick={() => setShowReviewCard(false)}>×</div>
                    </div>
                    <p className="review-card-body">
                        {courseObjectives.filter(o => !o.programOutcome).length > 0 &&
                            'Some COs are missing a Program Outcome mapping. '}
                        {courseObjectives.filter(o => o.bloomsTaxonomy.length === 0).length > 0 &&
                            "Some COs don't have Bloom's Taxonomy levels set. "}
                        Please complete before finalizing.
                    </p>
                    <button
                        className="review-card-btn"
                        onClick={() => {
                            const incomplete = courseObjectives.find(o => !o.programOutcome || o.bloomsTaxonomy.length === 0);
                            if (incomplete) setEditingCOId(incomplete.id);
                        }}
                    >
                        Complete Draft
                    </button>
            ...
            {/* ── Toast ────────────────────────────────────────────────── */}
            <div
                className={`notification-toast ${isToastVisible ? 'visible' : ''} ${toastType === 'success' ? 'toast-success' : toastType === 'error' ? 'toast-error' : 'toast-warning'}`}
            >
                <p>{toastMessage}</p>
            </div>
        </Layout>
